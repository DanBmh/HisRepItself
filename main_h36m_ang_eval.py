...
def main(opt):
    ...
    log.save_csv_log(
        opt, head, value, is_create=True, file_name="test_pre_action_256_seq"
    )
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (ang_h36) in enumerate(data_loader):
        ...
        ang_out_all = net_pred(
            ang_src, output_n=10, dct_n=opt.dct_n, itera=itera, input_n=in_n
        )
        ang_out_all = (
            ang_out_all[:, seq_in:]
            .transpose(1, 2)
        ...
        ang_gt_euler = (
            ang_h36[:, in_n : in_n + out_n].reshape([-1, 99]).reshape([-1, 3])
        )
        ...
        eulererr_ang_seq = torch.sum(
            torch.norm(ang_out_euler - ang_gt_euler, dim=2), dim=0
        )
