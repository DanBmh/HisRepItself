...
from model import GCN, AttModel
...
def main(opt):
    ...
    #
    net_pred_pose = AttModel.AttModel(
    ...
    net_pred_pose.cuda()
    model_path_len = (
        "./checkpoint/pretrained/h36m_3d_in50_out10_dctn20/ckpt_best.pth.tar"
    )
    ...
    net_pred_part = AttModel.AttModelPerParts(
        in_features=in_features,
        kernel_size=kernel_size,
        d_model=d_model,
        num_stage=opt.num_stage,
    ...
    net_pred_joint = AttModel.AttModelPerParts(
        in_features=in_features,
        kernel_size=kernel_size,
        d_model=d_model,
        num_stage=opt.num_stage,
    ...
    net_pred = GCN.GCN(
        input_feature=(kernel_size + opt.output_n) * 3,
...
def run_model(
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        with torch.no_grad():
            p3d_out_pose = net_pred_pose(
                p3d_src, input_n=in_n, output_n=out_n, itera=itera
            )[:, :, 0]
            p3d_out_part = net_pred_part(
                p3d_src, input_n=in_n, output_n=out_n, itera=itera
            )[:, :, 0]
            p3d_out_joint = net_pred_joint(
                p3d_src, input_n=in_n, output_n=out_n, itera=itera
            )[:, :, 0]
        p3d_in = torch.cat(
            [p3d_out_pose, p3d_out_part, p3d_out_joint], dim=1
        ).transpose(1, 2)
        ...
        p3d_out_all = (
            torch.matmul(out_est, p3d_out_cand)
            .squeeze(1)
            .reshape([batch_size, out_n, -1])
        )
