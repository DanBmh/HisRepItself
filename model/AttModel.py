...
class AttModel(Module):
    def __init__(
        self, in_features=48, kernel_size=5, d_model=512, num_stage=2, dct_n=10
    ):
        ...
        self.convQ = nn.Sequential(
            nn.Conv1d(
                in_channels=in_features, out_channels=d_model, kernel_size=6, bias=False
            ),
            nn.ReLU(),
            nn.Conv1d(
    ...
    def forward(self, src, output_n=25, input_n=50, itera=1):
        ...
        idx = np.expand_dims(np.arange(vl), axis=0) + np.expand_dims(
            np.arange(vn), axis=1
        )
        ...
        for i in range(itera):
            ...
            if itera > 1:
                ...
                src_dct_tmp = (
                    src_tmp[:, idx_dct].clone().reshape([bs * self.kernel_size, vl, -1])
                )
                src_dct_tmp = (
                    torch.matmul(dct_m[:dct_n].unsqueeze(dim=0), src_dct_tmp)
                    .reshape([bs, self.kernel_size, dct_n, -1])
                    .transpose(2, 3)
                    .reshape([bs, self.kernel_size, -1])
                )
...
class AttModelPerParts(Module):
    ...
    def forward(self, src, output_n=25, input_n=50, itera=1):
        ...
        for i in range(itera):
            ...
            for ii, pidx in enumerate(self.parts_idx):
                query_tmp.append(
                    self.convQ[ii](src_query_tmp[:, pidx] / 1000.0).unsqueeze(1)
                )
            ...
            for ii, pidx in enumerate(self.parts_idx):
                dct_att_tt = torch.matmul(
                    att_tmp[:, ii],
                    src_value_tmp[:, :, pidx].reshape([bs, -1, len(pidx) * self.dct_n]),
                ).squeeze(1)
            ...
            for ii, pidx in enumerate(self.parts_idx):
                key_new.append(
                    self.convK[ii](src_key_tmp[:, pidx] / 1000.0).unsqueeze(1)
                )
