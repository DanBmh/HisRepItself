...
def main(opt):
    ...
    net_pred = AttModel.AttModel(
        in_features=66, kernel_size=10, d_model=256, num_stage=12, dct_n=20
    )
    net_pred.cuda()
    print(
    ...
    for act in acts:
        ...
        # evaluation
        ret, nt = run_model(
            net_pred, is_train=3, data_loader=test_loader, opt=opt, good_idx=sele[act]
        )
        err += ret
        n += nt
    err = err / n
...
def run_model(
    net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None, good_idx=[]
):
    net_pred.eval()
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        mpjpe_p3d_h36 = torch.sum(
            torch.mean(torch.norm(p3d_h36[:, in_n:] - p3d_out_50, dim=3), dim=2), dim=0
        )
        m_p3d_h36[0] += mpjpe_p3d_h36.cpu().data.numpy()

        mpjpe_p3d_h36 = torch.sum(
            torch.mean(torch.norm(p3d_h36[:, in_n:] - p3d_out_100, dim=3), dim=2), dim=0
        )
